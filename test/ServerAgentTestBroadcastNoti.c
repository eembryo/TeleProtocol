...
static void
client_service_noti_callback (OpHandle handle, const IpcmdOperationInfo *info, gpointer cb_data)
{
	// handle will be 0x0
	IpcmdOperationInfoReceivedMessage *recv_info = (IpcmdOperationInfoReceivedMessage*)info;

	switch(IpcmdMessageGetVCCPDUOpType(recv_info->raw_message_)) {
	case IPCMD_OPTYPE_NOTIFICATION:
		g_message("%s: Got NOTIFICATION",__func__);
		break;
	case IPCMD_OPTYPE_NOTIFICATION_CYCLIC:
		g_message("%s: Got NOTIFICATION_CYCLIC",__func__);
		break;
	default:
		g_error("not happened");
	}
}
...
gint main()
{
	...
	IpcmdAgentTransportAddUdpv4Server (agent, TRANSPORT_LOCAL_ADDRESS, TRANSPORT_LOCAL_PORT, TRUE);
	...
	{
		IpcmdOperationCallback	noti_cb = {
				.cb_func = client_service_noti_callback,
				.cb_destroy = NULL,
				.cb_data = agent,
		};
		IpcmdAgentClientListenNotiUdpv4 (agent, CLIENT_REMOTE_SERVICE_ID, 0, TRUE, &noti_cb);
	}
