...
DECLARE_SM_ENTRY(DoAction_CLSetnor_Idle_SendSetnor)
{
	...
	// 1. create SETREQUEST_NORETURN message
	...
	// 3. change state
	op_state->state_ = kOpContextStateSetnorSent;
	// 4. Send IpcmdMessage
...
DECLARE_SM_ENTRY(DoAction_CLSetnor_Idle_RecvSetnor)
{
	IpcmdOpCtx *ctx = container_of(op_state, IpcmdOpCtx, mOpState);
	IpcmdMessage *message = (IpcmdMessage *)data;

	// 1. change state to kOpContextStateSetnorRecv
	op_state->state_ = kOpContextStateSetnorRecv;
...
DECLARE_SM_ENTRY(DoAction_CLSetnor_SetnorSent_RecvAck)
{
	IpcmdOpCtx *ctx = container_of(op_state, IpcmdOpCtx, mOpState);
...
DECLARE_SM_ENTRY(DoAction_CLSetnor_Any_RecvError)
{
	IpcmdOpCtx *ctx = container_of(op_state, IpcmdOpCtx, mOpState);
	IpcmdMessage *message = (IpcmdMessage *)data;
	...
	//3.Deliver ERROR message to application.
	{
		IpcmdOperationInfoReceivedMessage info;
		IpcmdOperationInfoReceivedMessageInit(&info);
		info.raw_message_ = IpcmdMessageRef(message);
...
DECLARE_SM_ENTRY(DoAction_CLSetnor_SetnorSent_WFAExpired)
{
	IpcmdOpCtx *ctx = container_of(op_state, IpcmdOpCtx, mOpState);
...
/* REQPROD 346846/MAIN;2 : UDP SETREQUEST_NORETURN
 * Hyobeom: In the figure, Client waits for ERROR response after it receives ACK message. It means that we should wait
 * at least a WFR timeout to ensure success of SETREQUEST_NORETURN operation.
 */
DECLARE_SM_ENTRY(DoAction_CLSetnor_AckRecv_WFRExpired)
{
	IpcmdOpCtx *ctx = container_of(op_state, IpcmdOpCtx, mOpState);
...
DECLARE_SM_ENTRY(DoAction_CLSetnor_AppProcess_CompleteAppProcess)
{
	IpcmdOpCtx 					*ctx = container_of(op_state, IpcmdOpCtx, mOpState);
	...
	//4. if there is ERROR message to send, send it and change state to kOpContextStateErrorSent
	if (error_message) {
...
DECLARE_SM_ENTRY(DoAction_CLSetnor_AppProcess_RecvSetnor)
{
	IpcmdOpCtx 					*ctx = container_of(op_state, IpcmdOpCtx, mOpState);
...
DECLARE_SM_ENTRY(DoAction_CLSetnor_ErrorSent_WFAExpired)
{
	IpcmdOpCtx *ctx = container_of(op_state, IpcmdOpCtx, mOpState);

	//1.if the number of retries exceeds maximum, finalize it
	ctx->numberOfRetries++;
	if (ctx->numberOfRetries > ctx->nWFAMaxRetries) {
		_SetFinalizeState (ctx, OPCONTEXT_FINCODE_EXCEED_MAX_RETRIES);
		return op_state->state_;
	}
	//retransmit ERROR message
	IpcmdCoreTransmit (ctx->core_, ctx->opctx_id_.channel_id_ , ctx->message);
	//reset timer
	IpcmdOpCtxSetTimer(ctx, CalculateTimeoutInterval(ctx->nWFABaseTimeout, ctx->nWFAIncreaseTimeout, ctx->numberOfRetries), ctx->OnWFAExpired);
...
DECLARE_SM_ENTRY(DoAction_CLSetnor_ErrorSent_RecvAck)
{
	IpcmdOpCtx *ctx = container_of(op_state, IpcmdOpCtx, mOpState);
...
void
IpcmdOpStateMachine_CLSetnor_Init(IpcmdOpStateMachine *SM)
{
	guint ns,nt;
	...
	SM->actions[kOpContextStateIdle][kIpcmdTriggerSendSetnor] = DoAction_CLSetnor_Idle_SendSetnor;
	SM->actions[kOpContextStateIdle][kIpcmdTriggerRecvSetnor] = DoAction_CLSetnor_Idle_RecvSetnor;
	SM->actions[kOpContextStateSetnorSent][kIpcmdTriggerRecvAck] = DoAction_CLSetnor_SetnorSent_RecvAck;
	SM->actions[kOpContextStateSetnorSent][kIpcmdTriggerRecvError] = DoAction_CLSetnor_Any_RecvError;
	SM->actions[kOpContextStateSetnorSent][kIpcmdTriggerWFATimeout] = DoAction_CLSetnor_SetnorSent_WFAExpired;
	SM->actions[kOpContextStateAckRecv][kIpcmdTriggerRecvError] = DoAction_CLSetnor_Any_RecvError;
	SM->actions[kOpContextStateAckRecv][kIpcmdTriggerWFRTimeout] = DoAction_CLSetnor_AckRecv_WFRExpired;
	SM->actions[kOpContextStateAckRecv][kIpcmdTriggerRecvAck] = DoAction_Ignore;
//	SM->actions[kOpContextStateSetnorRecv][] = ;
	SM->actions[kOpContextStateAppProcess][kIpcmdTriggerCompletedAppProcess] = DoAction_CLSetnor_AppProcess_CompleteAppProcess;
	SM->actions[kOpContextStateAppProcess][kIpcmdTriggerRecvSetnor] = DoAction_CLSetnor_AppProcess_RecvSetnor;
	SM->actions[kOpContextStateErrorSent][kIpcmdTriggerWFATimeout] = DoAction_CLSetnor_ErrorSent_WFAExpired;
	SM->actions[kOpContextStateErrorSent][kIpcmdTriggerRecvAck] = DoAction_CLSetnor_ErrorSent_RecvAck;
	SM->actions[kOpContextStateErrorSent][kIpcmdTriggerRecvSetnor] = DoAction_Ignore;	//IMPL: retransmit ERROR message
}
